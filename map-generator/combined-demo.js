...
function drawMap() {
    ...
    // Debug log to check if Hawaii is in the features
    const hawaiiFeature = statesFeatures.find(d => d.properties.code === 'HI');
    console.log('Hawaii feature:', hawaiiFeature);
    ...
    svg.selectAll('path')
        ...
        .append('path')
        .attr('d', d => {
            // Debug log for Hawaii path
            if (d.properties.code === 'HI') {
                console.log('Hawaii path data:', pathGenerator(d));
            }
        ...
        .attr('fill', d => {
            ...
            // Debug log for Hawaii fill
            if (stateCode === 'HI') {
                console.log('Hawaii fill:', stateDataItem ? colorScale(stateDataItem.value) : '#eee');
                console.log('Hawaii data item:', stateDataItem);
            }
            ...
            if (stateCode === 'HI') {
                console.log('Forcing Hawaii to be colored');
            ...
            if (stateDataItem) {
                // Make sure zero values are properly colored using the scale
                if (stateDataItem.value === 0) {
                    console.log('Coloring state with zero value:', stateCode);
                }
    ...
    if (smallStates.length > 0) {
        ...
        // No background rectangle or title to match the example photo
    ...
    if (stateData.length > 0) {
        ...
        // No background for the legend area to make it blend with the map
        ...
        // Debug log to see what value is being used for the min label
        console.log('Drawing min label with value:', legendMinLabel);
        ...
        // Debug log to see what value is being used for the max label
        console.log('Drawing max label with value:', legendMaxLabel);
...
function setupEventListeners() {
    ...
    document.getElementById('apply-settings').addEventListener('click', () => {
        ...
        // Debug log to see if the values are being updated
        console.log('Updated legend labels:', { legendMinLabel, legendMaxLabel });
    ...
    try {
        ...
        const processedData = rawData
            ...
            .map(row => {
                ...
                // Special case for Hawaii - check for common variations
                if (stateCode === 'HAWAII' || stateCode === 'HI' || stateCode === 'H' || stateCode.includes('HAWAII')) {
                    stateCode = 'HI';
                    console.log('Found Hawaii in data:', row);
                }
                ...
                if (row[valueColumn] !== undefined && row[valueColumn] !== '') {
                    ...
                    // Special case for zero values - make sure they're treated as valid numbers
                    if (value === 0) {
                        console.log('Found zero value for state:', stateCode);
                    }
                }

                // Special case for Hawaii - always include it even with null value
                if (stateCode === 'HI') {
                    console.log('Processing Hawaii with value:', value);
                }
        ...
        // Debug log to check if Hawaii data is present
        const hawaiiData = processedData.find(item => item.stateCode === 'HI');
        console.log('Hawaii data:', hawaiiData);
        ...
        // Log the custom colors for debugging
